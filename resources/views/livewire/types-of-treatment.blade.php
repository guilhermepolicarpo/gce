<div class="p-6 sm:px-10 bg-white border-b border-gray-200">
                    ...
                    <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider flex align-center ">
                                ...
                                @forelse ($typesOfTreatment as $typeOfTreatment)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <button wire:click="confirmTypeOfTreatmentEditing({{ $typeOfTreatment->id }})" class="text-indigo-600 hover:text-indigo-900 mr-3">
                                            ...
                                            <div class="flex items-center">
                                                <div>
                                                    <div class="text-base font-medium text-gray-500">
                                                        Nenhum Tipo de Tratamento encontrado.
                                                    </div>
                                                </div>
                                            </div>
                                        </td>
    ...
    <x-jet-confirmation-modal wire:model="confirmingTypeOfTreatmentDeletion">
        <x-slot name="title">
            {{ __('Deletar tipo de tratamento') }}
        </x-slot>

        <x-slot name="content">
            {{ __('Tem certeza de que deseja excluir este tipo de tratamento?') }}
        </x-slot>
    ...
    <x-jet-dialog-modal wire:model="confirmingTypeOfTreatmentAddition" maxWidth="lg">
        <x-slot name="title">
            {{ ($this->actionAdd) ? __('Adicionar tipo de tratamento') : __('Editar tipo de tratamento') }}
        </x-slot>
